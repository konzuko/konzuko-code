...
serve(async (req: Request): Promise<Response> => {
  ...
  if (authErr || !user) return json({ error: authErr?.message ?? 'Unauthenticated' }, 401)
  ...
  try {
    /* GET = fetch stored key */
    if (req.method === 'GET') {
      const { data, error } = await supabaseAdmin.rpc('get_user_api_key', {
        p_user_id: user.id,
      })
      if (error?.code === '42883') return json({ apiKey: '' })   // RPC not yet created
    ...
    if (req.method === 'POST') {
      ...
      if (typeof apiKey !== 'string') return json({ error: '`apiKey` must be a string' }, 400)
      ...
      if (error?.code === '42883') {                // migration missing → fall back
